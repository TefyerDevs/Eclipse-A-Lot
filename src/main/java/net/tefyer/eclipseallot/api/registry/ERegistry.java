...
public abstract class ERegistry<K, V> implements Iterable<V> {

    public static final Map<ResourceLocation, ERegistry<?, ?>> REGISTERED = new HashMap<>();
    ...
    public ERegistry(ResourceLocation registryName) {
    ...
    private boolean checkActiveModContainerIsGregtech() {
        ...
        return container != null && (container.getModId().equals(this.registryName.getNamespace()) ||
                container.getModId().equals(Eclipseallot.MODID) ||
    ...
    public <T extends V> T replace(K key, T value) {
        ...
        if (!containKey(key)) {
            Eclipseallot.LOGGER.warn("[replace] couldn't find key %s in registry %s".formatted(registryName, key));
        }
        registry.put(key, value);
        return value;
    }
    ...
    public static class RL<V> extends ERegistry<ResourceLocation, V> {
